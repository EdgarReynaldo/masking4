...
void MAS::CheckBox::MsgInitSkin() {
	Button::MsgInitSkin();
	for (int i=0; i<4; i++) {
		if (GetFontColor(i) == -1) SetFontColor(skin->fcol[Skin::INFO_CHECKBOX][i], skin->scol[Skin::INFO_CHECKBOX][i], i);
		if (GetFontIndex(i) == -1) SetFont(skin->fnt[Skin::INFO_CHECKBOX][i], i);
	}
	if (GetTextMode() == -1) SetTextMode(skin->c_face);
...
void MAS::CheckBox::Draw(Bitmap &canvas) {
	...
	bmp.MaskedBlit(canvas, ox*bw, oy*bh, box_x1, box_y1, bw, bh);
