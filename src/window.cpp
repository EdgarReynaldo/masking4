...
void MAS::FrameBorder::Draw(Bitmap &canvas) {
	...
	switch (type) {
		case 0:
		case 1:
			bmp.MaskedVerticalTile(canvas, off, 0, 0, h(), 2);
			break;

		case 2:
		case 3:
		case 4:
			bmp.MaskedHorizontalTile(canvas, off, 0, 0, w(), 2);
			break;
	}
}
...
void MAS::TextBack::Draw(Bitmap &canvas) {
	Bitmap bmp = GetBitmap();
	bmp.MaskedHorizontalTile(canvas, HasFocus() ? 0 : 1, 0, 0, w(), 2);
}
...
void MAS::Window::UpdateSize() {
	...
	// make sure the dialog below is redrawn in case the borders have transparency in them
	Root()->Redraw(bLeft);
	Root()->Redraw(bRight);
	Root()->Redraw(bTop);
	Root()->Redraw(bBottom);
...
void MAS::Window::Autorollup(bool a) {
	if (a) {
		...
		if (!HasFocus() && !(extraFlags & W_MINIMIZED) && driver) {
			Minimize();
		}
	}
	else {
		extraFlags &= ~W_AUTOROLLUP;
		if ((extraFlags & W_MINIMIZED) && driver) {
			Minimize();
		}
	}
}
...
void MAS::Window::HandleEvent(MAS::Widget &obj, int msg, int arg1, int arg2) {
...
void MAS::WinClientArea::HandleEvent(Widget &obj, int msg, int arg1, int arg2) {
