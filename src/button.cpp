...
void MAS::Button::MsgInitSkin() {
	Widget::MsgInitSkin();
	for (int i=0; i<4; i++) {
		if (GetFontColor(i) == -1) SetFontColor(skin->fcol[Skin::INFO_BUTTON][i], skin->scol[Skin::INFO_BUTTON][i], i);
...
void MAS::Button::MsgKey() {
	...
	counter = (MAS::Settings::keyDelay > 0 ? MAS::Settings::keyDelay+20 : 300)*MAS::Settings::logicFrameRate/1000;
