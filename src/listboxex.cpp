...
void ListBoxEx::Header::Draw(Bitmap &canvas) {
	...
	// for all items:
	int index = 0;
	for (vector<Column>::iterator i = columns->begin(); i != columns->end(); ++i, index++) {
...
void ListBoxEx::Header::DrawItemBackground(Bitmap &canvas, Column *item, int state, const Rect &dstRect) {
	...
	if (bmp) {
		bmp.MaskedTiledBlit(canvas, 0, state, dstRect.x(), dstRect.y(), dstRect.w(), dstRect.h(), 1, 4);
	}
}
...
ListBoxEx::Item::Item(const ListBoxEx::Item &rhs, long data) {
	cells = rhs.cells;
	parent = rhs.parent;
...
bool ListBoxEx::Item::operator<(const ListBoxEx::Item &rhs) const {
	return (ustricmp(GetText(sortColumn), rhs.GetText(sortColumn))*sortOrder < 0) ? true : false;
}
...
void ListBoxEx::Item::DrawBackground(Bitmap &canvas, int state, const Rect &dstRect) {
	Bitmap bmp = parent->GetBitmap();
	if (bmp) {
		bmp.MaskedTiledBlit(canvas, 0, state, dstRect.x(), dstRect.y(), dstRect.w(), dstRect.h(), 1, 8);
	}
}
...
void ListBoxEx::Item::DrawGrid(Bitmap &canvas, int i, int state, const Rect &dstRect) {
	...
	for (int x = dstRect.x(); x <= x2; x += 2) {
		putpixel(canvas, x, y2, col);
	}

	for (int y = dstRect.y(); y <= y2; y += 2) {
		putpixel(canvas, x2, y, col);
	}
}
...
void ListBoxEx::Item::InsertColumn(int index) {
	if (index < 0 || index > cells.size()) {
		...
		vector<string>::iterator i = cells.begin();
		for (int j=0; j<index; ++j, ++i);
		cells.insert(i, string(""));
	}
}
...
void ListBoxEx::Item::SetText(const char *str, int index) {
	if (index >= 0 && index < cells.size()) {
		cells[index] = str ? string(str) : string("");
	}
}
...
void ListBoxEx::MsgInitSkin() {
	...
	for (int i=0; i<4; i++) {
		if (GetFontColor(i) == -1) SetFontColor(skin->fcol[Skin::INFO_LIST][i], skin->scol[Skin::INFO_LIST][i], i);
...
int ListBoxEx::InsertColumn(const char *title, int index, int align, int width) {
	...
	Column col;
	col.title = title ? string(title) : string("");
	col.alignment = align;
	col.width = width;
	col.sortOrder = 1;
	...
	if (index < 0 || index > columns.size()) {
		...
		vector<Column>::iterator i = columns.begin();
		for (int j=0; j<index; ++j, ++i);
...
void ListBoxEx::DeleteColumn(int index) {
	...
	if (index < columns.size() && index >= 0) {
		...
		if ((columns.size() < 1 && string(GetColumnTitle(0)) == "") || !showHeader) {
			if (!header->Hidden()) {
				header->Hide();
				SetFlag(D_RESIZED);
			}
...
void ListBoxEx::SetColumnTitle(int index, const char *title) {
	if (index < columns.size() && index >= 0) {
		columns[index].title = string(title);
	}
}
...
int ListBoxEx::InsertItem(Item *item, int index) {
	...
	while (item->GetColumnCount() < columns.size()) {
		item->InsertColumn();
	}
...
void ListBoxEx::Deselect(int i) {
	if (i >= 0 && i < items.size() && multiSelect) {
		items[i]->Deselect();
	}
}
...
int ListBoxEx::GetFirstSelectedIndex() {
	...
	if (multiSelect) {
		...
		int index = 0;
		for (vector<Item *>::iterator i = items.begin(); i != items.end(); ++i, index++) {
			if ((*i)->IsSelected()) {
...
int ListBoxEx::GetNextSelectedIndex() {
	if (multiSelect) {
		...
		for (; iterationIndex < items.size(); iterationIndex++) {
...
void ListBoxEx::HandleEvent(Widget &obj, int msg, int arg1, int arg2) {
...
void ListBoxEx::OnMousemove() {
	...
	if (focusedItem != oldFocus) {
		...
		if ((GetMouse()->flags & 1) && inputHandler.HasFocus() && inputHandler.HasMouse()) {
			if (multiSelect) {
				Toggle(focusedItem);
			}
			else {
...
void ListBoxEx::OnChar(int c) {
	...
	// handle keys
	switch (c>>8) {
		case KEY_UP:		--newFocus;					break;
		case KEY_DOWN:		++newFocus;					break;
		case KEY_PGUP:		newFocus -= 16;				break;
		case KEY_PGDN:		newFocus += 16;				break;
		case KEY_HOME:		newFocus = 0;				break;
		case KEY_END:		newFocus = items.size();	break;
...
void ListBoxEx::OnWheel(int z) {
	...
	index = MID(0, index, items.size()-1);
	if (index >= 0 && index < items.size()) {
		int old = GetYOffset();
