...
void MAS::Scroller::OriginalHandleEvent(Widget& obj, int msg, int arg1, int arg2) {
	if (msg == MSG_ACTIVATE) {
		if (obj == bUp) {
			double oldpos = pos;
			Decrement();
			if (pos != oldpos) {
				...
				GetParent()->HandleEvent(*this, MSG_SCROLL, (int)pos);
			}
		}
		else if (obj == bDown) {
			double oldpos = pos;
		...
		switch (arg1) {
			case KEY_LEFT:	if (orientation == 1) newpos -= d;	break;
			case KEY_RIGHT:	if (orientation == 1) newpos += d;	break;
			case KEY_UP:	if (orientation == 0) newpos -= d;	break;
			case KEY_DOWN:	if (orientation == 0) newpos += d;	break;
			case KEY_PGUP:	newpos -= visibleItems;				break;
			case KEY_PGDN:	newpos += visibleItems;				break;
			case KEY_HOME:	newpos = min;						break;
			case KEY_END:	newpos = max;						break;
			case -1: {
 ...
 void MAS::Scroller::HandleEvent( MAS::Widget & obj, int msg, int arg1, int arg2 ) {
   if ( (hInc == -1) && (vInc == -1) ) {
   ...
   if ( msg == MSG_ACTIVATE ) {
     if ( obj == bUp ) {
       arg1 = (orientation == 1) ? KEY_LEFT : KEY_UP;  goto vkScroll;
     }
     else if ( obj == bDown ) {
       arg1 = (orientation == 1) ? KEY_RIGHT : KEY_DOWN;  goto vkScroll;
     }
     else if ( obj == back ) {
       if ( !bGrip.HasMouse() ) {
         Point mp = GetMouse()->pos;
         if ( orientation == 1 ) {
           if ( mp.x() < bGrip.x() ) {
             arg1 = KEY_LEFT;  goto vkScroll;
           }
           else if ( mp.x() > bGrip.x2() ) {
             arg1 = KEY_RIGHT;  goto vkScroll;
           }
         }
         else {
           if ( mp.y() < bGrip.y() ) {
             arg1 = KEY_PGUP;  goto vkScroll;
           }
           else if ( mp.y() > bGrip.y2() ) {
             arg1 = KEY_PGDN;  goto vkScroll;
           }
         }
       }
     }
 ...
 vkScroll:
     ...
     double newpos = pos;
     switch ( arg1 ) {
       case KEY_LEFT:  if (orientation == 1) newpos -= hInc;  break;
       case KEY_RIGHT: if (orientation == 1) newpos += hInc;  break;
       case KEY_UP:  if (orientation == 0) newpos -= vInc;  break;
       case KEY_DOWN:  if (orientation == 0) newpos += vInc;  break;
       case KEY_PGUP:  newpos -= visibleItems;       break;
       case KEY_PGDN:  newpos += visibleItems;       break;
       case KEY_HOME:  newpos = min;           break;
       case KEY_END: newpos = max;           break;
...
void MAS::ScrollBack::Draw(Bitmap &canvas) {
	...
	GetBitmap().MaskedTiledBlit(canvas, 0, state, 0, 0, w(), h(), 1, 4);
}
...
void MAS::ScrollBack::MsgTick() {
	...
	if (TestFlag(D_PRESSED)) {
		...
		if (m->pos <= *this) {
			/*
			if ( (((MAS::Scroller *)parent)->GetHorizontalIncrement() != -1) || (((MAS::Scroller *)parent)->GetVerticalIncrement() != -1) )
				ClearFlag(D_PRESSED);
			*/
...
bool MAS::ScrollButton::MsgChar(int c) {
	switch (c>>8) {
		case KEY_LEFT:
			GetParent()->HandleEvent(*this, MSG_SCROLL, KEY_LEFT);
			break;

		case KEY_RIGHT:
			GetParent()->HandleEvent(*this, MSG_SCROLL, KEY_RIGHT);
			break;

		case KEY_UP:
			GetParent()->HandleEvent(*this, MSG_SCROLL, KEY_UP);
			break;

		case KEY_DOWN:
			GetParent()->HandleEvent(*this, MSG_SCROLL, KEY_DOWN);
			break;

		case KEY_PGUP:
			GetParent()->HandleEvent(*this, MSG_SCROLL, KEY_PGUP);
			break;

		case KEY_PGDN:
			GetParent()->HandleEvent(*this, MSG_SCROLL, KEY_PGDN);
			break;

		case KEY_HOME:
			GetParent()->HandleEvent(*this, MSG_SCROLL, KEY_HOME);
			break;

		case KEY_END:
			GetParent()->HandleEvent(*this, MSG_SCROLL, KEY_END);
			break;
