...
void MAS::Animator::Create(Bitmap &bmp, int length) {
	if (!buffer || (bmp.w() != buffer.w() || bmp.h() != buffer.h())) {
		...
		bmp.Blit(buffer, 0, 0, 0, 0, bmp.w(), bmp.h());

		back.Destroy();
		back.Create(bmp.w(), bmp.h());
		bmp.Blit(back, 0, 0, 0, 0, bmp.w(), bmp.h());
	}
	else if (!back) {
		back.Destroy();
		back.Create(bmp.w(), bmp.h());
...
void MAS::Animator::Draw(Bitmap &canvas, int type) {
	...
	switch (type) {
		...
		case BLEND: {
			set_trans_blender(0,0,0,(progress<<8)*(progress)/(frames*frames));
			canvas.DrawTransSprite(buffer, 0, 0);
			break;
		}
